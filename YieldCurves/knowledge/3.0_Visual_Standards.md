...
### 2b. Duration X-Axis
When X-axis mode is **Duration**, a point is positioned by the modified duration of the security rather than by its maturity date or time-to-maturity. Duration is computed from the market price with the shared `calculateMDuration` (Actual/Actual, semi-annual) and covers both TIPS (at their real yield) and nominal Treasuries; nothing is read from a stored field. Bills and STRIPS resolve to their time-to-maturity, the limiting case of the same formula.

- **Per-series yield**: each series computes its own duration from its own yield, so the Ask, SA and SAO points for one TIPS can sit at slightly different positions along the axis. This is unlike Maturity and Term modes, where a bond's position is a single structural fact shared by all its series.
- **Tick engine**: Duration reuses the Term (TTM) linear scale and its adaptive-granularity tick and label rules (§2a) unchanged — the position fed to that scale is modified-duration-years in place of weeks-to-maturity. Tooltips report the value as `Duration X.XXy`.
- **Ordering**: each series is drawn in ascending duration order so the connecting line does not cross itself where duration order departs from maturity order.
- **Degenerate points**: a security whose duration cannot be computed (yield at or below −200%, or no settlement date) is omitted from the Duration view.

**BEI leg pairing** follows the X-axis mode. In Maturity and Term modes each TIPS is paired to the closest-maturity nominal (closest time-to-maturity is the same ordering). In Duration mode it is paired to the closest-modified-duration nominal instead — the TIPS taken at its Ask (raw real) yield against the nominal at its ask yield — and the BEI rows are ordered by increasing TIPS duration. A TIPS carrying a low real coupon has a longer duration than a same-maturity nominal carrying a market coupon, so the two pairings disagree, and the reported breakeven for a given TIPS differs between the Duration view and the Maturity or Term view.
