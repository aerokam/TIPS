...
- <a id="s13"></a>**S13: SpotYieldCurves.json** = `{ @curve + @source + settlementDate + asOf + beta0 + beta1 + beta2 + beta3 + tau1 + tau2 + tMin + tMax }`
  *The [spot yield](../YieldCurves/knowledge/4.0_Spot_Yield_Curves.md) Svensson fits YieldCurves computes but does not otherwise persist: `curve` is `nominal`, `tips_ask` (fit to the quoted ask price), or `tips_sa` (fit to the [seasonally adjusted](#sa-factor) price); `source` is `FedInvest` or `Market`. Six parameters and nothing evaluated, same convention as [S12](#s12) — consumers evaluate `z(t)` themselves via `shared/src/spot-curve.js`. Spot breakeven inflation is not stored separately: it is `nominal/Market` minus `tips_sa/Market` at each horizon, fully re-derivable from the two rows already here. Written by `YieldCurves/scripts/updateSpotYieldCurves.js`. R2 key: `Treasuries/SpotYieldCurves.json`.*
