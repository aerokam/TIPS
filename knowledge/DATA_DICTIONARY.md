...
- <a id="s11"></a>**S11: FundHoldings/Holdings-\<TICKER\>(-Enriched).csv** — Vanguard/fminvest/PIMCO/Schwab/BondBloxx fund holdings (E7/E8/E9/E10/E11), raw and enriched, one pair per fund ticker (VBIL, VTIP, VTP, RBIL, LTPZ, SCHP, XHLF). Own top-level R2 prefix (`FundHoldings/`) rather than nested under `TIPS/`/`Treasuries/`: a single fund's holdings CSV mixes TIPS and nominal rows (discriminated downstream by CUSIP presence in S10, not a column flag), so it doesn't belong to either instrument-type prefix — same rationale as `misc/`. Also stores `FundHoldings/FundMeta.json` (`{ @Ticker: { fundName, portId | etfId | cusip } }`). Written by `FundHoldings/updateAllHoldings.js`; fetched directly by `FundHoldings/index.html` client-side.
