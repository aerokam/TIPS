...
- <a id="s13"></a>**S13: SpotYieldCurves.csv** = `{ term_years + ( maturity_date + @cusip + security_type ) + source + ( ask_yield + sa_yield + sao_yield ) + ( spot_yield + spot_sa_yield + bei ) }`
  *Evaluated yields, general-purpose and spreadsheet-ready — every priced Treasury and TIPS security (`cusip`/`maturity_date`/`security_type` populated, `ask_yield`/`sa_yield`/`sao_yield` populated where they exist) plus the fitted nominal and TIPS-SA [spot yield](../YieldCurves/knowledge/4.0_Spot_Yield_Curves.md) curves evaluated on a half-year term grid (`cusip`/`maturity_date`/`security_type` blank, `spot_yield`/`spot_sa_yield`/`bei` populated). `source` is `FedInvest` or `Market`. `bei` = `spot_yield − spot_sa_yield` (nominal spot yield minus real/SA spot yield) at each grid point, per [4.0 Spot Yield Curves §Spot BEI](../YieldCurves/knowledge/4.0_Spot_Yield_Curves.md#spot-bei) — computed directly from the two fitted values already on that row, not a second implementation. Replaces the parameters-only `SpotYieldCurves.json` (retired 2026-09-07): six Svensson coefficients aren't usable in a spreadsheet, so this file stores actual yields via `shared/src/spot-curve.js` instead of the unevaluated parameters [S12](#s12) still uses. Written by `YieldCurves/scripts/updateSpotYieldCurves.js`. R2 key: `Treasuries/SpotYieldCurves.csv`.*
