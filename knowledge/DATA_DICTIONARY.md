...
- <a id="s11"></a>**S11: FundHoldings/Holdings-\<TICKER\>(-Enriched).csv** — Vanguard/fminvest fund holdings (E7/E8), raw and enriched, one pair per fund ticker (VBIL, VTIP, VTP, RBIL). Own top-level R2 prefix (`FundHoldings/`) rather than nested under `TIPS/`/`Treasuries/`: a single fund's holdings CSV mixes TIPS and nominal rows (discriminated downstream by CUSIP presence in S10, not a column flag), so it doesn't belong to either instrument-type prefix — same rationale as `misc/`. Also stores `FundHoldings/FundMeta.json` (`{ @Ticker: { fundName, portId | etfId } }`). Written by `FundHoldings/updateAllHoldings.js`; fetched directly by `FundHoldings/index.html` client-side.
